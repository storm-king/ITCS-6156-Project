...
class CocoDataset():
    ...
    def display_captions(self, image_id):
        print('Captions:')
        print('==========')
        if len(self.captions) == 0 or image_id not in self.captions:
            print("No Captions.")
        else: 
    ...
    def process_captions(self):
        self.captions = {}
        for caption in self.coco['annotations']:
            try:
                caption_id = caption['caption']
                image_id = caption['image_id']
                if image_id not in self.captions:
                    self.captions[image_id] = {caption_id}
                else:
                    self.captions[image_id] |= {caption_id}
            except:
                pass
